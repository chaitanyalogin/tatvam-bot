...
const state = {
  lastOffer: null,       // e.g. "etl"|"eol"|"projects"|...
  lastTopic: null,
...
function tryMath(s){
  ...
  // 🚫 avoid treating plain years like "2025" as math
  if(/^\d{4}$/.test(expr)) return null;

  try{ const v=eval(expr); if(Number.isFinite(v)) return "= "+(Number.isInteger(v)?v:v.toString()); }catch(_){}
  return null;
}

// ========= web summaries (improved) =========
// Force-search prefix support: "search: <query>" or "google: <query>"
...
// 🔎 Make auto-search trigger more often
function shouldWebSearch(q){
  const t=norm(q);
  if(getForcedQuery(q)) return true; // forced

  const words=t.split(" ").filter(w=>w && !STOPWORDS.has(w));
  const hasQuestionWord = /(what|who|when|where|why|how|define|meaning|explain)\b/.test(t);
  const endsQuestion = /\?$/.test(q.trim());
  const hasNumber = /\d/.test(t);

  // trigger if: typical question OR ends with ? OR has a number and 2+ words (e.g., "india population 2025")
  return hasQuestionWord || endsQuestion || (hasNumber && words.length>=2);
}
...
async function respond(q){
  ...
  const t=norm(forced || q); // normalise forced query if present
  ...
  if(forced || shouldWebSearch(q)){
    const query = forced || q;
    const w = await webSummary(query);
    if(w) return w;
  }
